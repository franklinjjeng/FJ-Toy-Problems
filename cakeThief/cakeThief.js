...
var maxDuffelBagValue = function(cakeTypes, capacity) {
  for (var i = 0; i < cakeTypes.length; i++) {
    cakeTypes[i].ratio = cakeTypes[i].value / cakeTypes[i].weight;
  }

  cakeTypes.sort((a, b) => b.ratio - a.ratio);

  var index = 0;
  var value = 0;
  var weight = 0;
  while (index < cakeTypes.length && weight < capacity) {
    if (weight + cakeTypes[index].weight <= capacity) {
      weight += cakeTypes[index].weight;
      value += cakeTypes[index].value;
    } else {
      index++;
    }
  }
